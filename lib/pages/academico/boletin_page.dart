...
class BoletinPageState extends State<BoletinPage> {
  ...
  Widget _buildLoadingState() {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Boletín de cursos'),
      ),
      body: const SimpleLoadingBodyWidget(),
    );
  }
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text(withMask
              ? 'Accediendo...'
              : 'Boletín ${_bloc!.semestres[_bloc!.selectedSemestreIndex!]}'),
          actions: withMask
              ? <Widget>[]
              : <Widget>[
                  // _buildInfoAction(),
  ...
  Widget _buildBody() {
    return ListaCursosWidget(
      ...
      nadaQueMostrarMensaje:
          'Puede que no tengas cursos inscritos en este semestre'
  ...
  Widget _buildSemestresAction() {
    return PopupMenuButton<int>(
      ...
      onSelected: (selectedIndex) {
        ...
        //     .registrarUso(CasosDeUso.BoletinSeleccionarSemestre);
  ...
  void _lanzarNotasPage() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => NotasPage(_bloc!.notasModel)),
    );
  }
